...
<div class="mb-6">
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2">
            ...
            <li>
                <span class="text-gray-900 font-medium">Edit {{ $FAQ->title }}</span>
            </li>
            <li class="ml-auto">
...
<div class="bg-white shadow rounded-lg">
    <div class="px-6 py-4 border-b border-gray-200">
        <div>
            <h3 class="text-lg font-semibold text-gray-900">Edit FAQ: {{ $FAQ->title }}</h3>
            <p class="text-sm text-gray-500 mt-1">Update frequently asked question</p>
        </div>
    </div>
    
    <form method="POST" action="{{url('/')}}/admin/edit_Faq/{{$FAQ->id}}" enctype="multipart/form-data" class="px-6 py-6">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                @php
                    $CategorySelected = DB::table('categories')->where('id', $FAQ->category)->first();
                    $categoryOptions = [];
            ...
            <div>
                <label for="article-ckeditor" class="block text-sm font-medium text-gray-700 mb-1">
                    FAQ Content <span class="text-red-500">*</span>
                </label>
                <textarea 
                    id="article-ckeditor" 
                    name="content" 
